...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  // Try to find the service in navigationData
  let serviceLabel =
    slug.charAt(0).toUpperCase() + slug.slice(1).replace(/-/g, " ");

  // This is a simplified lookup
  const name = `${serviceLabel} | ${CONTACT_INFO.companyName}`;
  const description = `Leading ${serviceLabel} services in Pune. Transform your brand with Growthik Media's expert solutions.`;
  ...
  return {
    ...
    openGraph: {
      title: name,
      description: description,
      url: `${CONTACT_INFO.website}/services/${slug}`,
    },
    alternates: {
...
export default async function ServiceDetailPage({ params }: Props) {
  ...
  // For now we just show a professional placeholder that matches the theme

  let serviceName = slug
    .split("-")
